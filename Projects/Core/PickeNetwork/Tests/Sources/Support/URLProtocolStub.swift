...
/// 정적 상태를 사용하므로 이 스텁을 쓰는 테스트 스위트는 직렬로 실행해야 한다.
final class URLProtocolStub: URLProtocol {
  ...
  /// baseURL 은 엔드포인트의 `domain` 이 들고 있으므로 클라이언트는 세션만 갈아끼운다.
  static func makeClient(authorizing: AuthorizingInterceptor? = nil) -> NetworkClient {
  ...
  override func startLoading() {
    ...
    let response = HTTPURLResponse(
      url: request.url ?? URL(string: "https://picke.test")!,
