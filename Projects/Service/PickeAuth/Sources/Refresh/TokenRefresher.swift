...
struct TokenRefresher: TokenRefreshing {
  /// 인증 헤더 없이 refresh endpoint 를 호출하는 요청 클라이언트다.
  private let client: any PickeRequestClient
  /// 서버가 credential 을 거부했을 때 인증 세션을 종료하는 콜백이다.
  private let onAuthFailure: @Sendable () async -> Void

  /// refresh 요청과 인증 실패 후처리를 한 갱신기로 조립한다.
  init(
    client: any PickeRequestClient,
  ...
  /// 현재 refresh token 을 새 credential 쌍으로 교환한다.
  func refresh(_ current: PickeCredential) async throws(PickeNetworkError) -> PickeCredential {
    do {
      let response = try await client.send(
        AuthService.refresh(refreshToken: current.refreshToken),
  ...
  private static func shouldEndSession(for error: PickeNetworkError) -> Bool {
    switch error {
    case let .response(response):
      // 5xx 는 서버 장애, 그 외 응답 오류는 토큰 거부로 구분한다.
...
private struct RefreshTokenResponse: Decodable, Sendable {
  /// refresh 응답에서 받은 새 access token 이다.
  let accessToken: String
  /// 다음 갱신에 사용할 새 refresh token 이다.
