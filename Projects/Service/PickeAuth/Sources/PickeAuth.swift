...
actor PickeAuth: AuthService, AuthenticatedClientProvider {
  ...
  /// 저장소에 사용할 수 있는 credential 이 남아 있는지 반환한다.
  var isLoggedIn: Bool {
    store.load() != nil
  }
  ...
  func signIn(accessToken: String, refreshToken: String) {
    store.allowSaves()
    let credential = PickeCredential(
  ...
  /// 영속 credential 과 실행 중인 네트워크 세션을 함께 비운다.
  func signOut() {
    store.clear()
